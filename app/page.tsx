...
export default function Home() {
  ...
  useEffect(() => { loadAtlas().then((loaded) => { setAtlas(loaded); setIsLiveData(true); }).catch(() => undefined); }, []);
  ...
  return (
    <main className="app-shell">
      <header className="topbar"><div className="brand-lockup"><span className="brand-mark"><span /></span><span>Perfect Weather</span></div><div className="topbar-center"><span className="eyebrow">Global climate atlas</span><span className="data-status"><i className={isLiveData ? 'is-live' : ''} /> {isLiveData ? 'ERA5 data loaded' : 'Loading climate grid'}</span></div><button className="topbar-button" onClick={() => setMobileControlsOpen((open) => !open)} aria-expanded={mobileControlsOpen}>Criteria <span className="sliders-icon">☷</span></button></header>
      <div className="app-grid">
        <aside className={`control-panel ${mobileControlsOpen ? 'is-open' : ''}`}>
          ...
          <div className="criteria-section season-section"><div className="section-label"><span>Time of year</span><span className="section-number">02</span></div><div className="criteria-range__heading"><span>Include months</span><strong>{MONTHS[criteria.monthStart - 1]}–{MONTHS[criteria.monthEnd - 1]}</strong></div><div className="month-pills">{MONTHS.map((month, index) => <button key={month} className={index + 1 >= criteria.monthStart && index + 1 <= criteria.monthEnd ? 'is-selected' : ''} onClick={() => { const monthValue = index + 1; setCriteria((current) => monthValue < current.monthStart ? { ...current, monthStart: monthValue } : monthValue > current.monthEnd ? { ...current, monthEnd: monthValue } : { ...current, monthStart: monthValue, monthEnd: monthValue }); }}>{month}</button>)}</div><div className="criteria-range__limits"><span>{selectedDayCount} days in view</span><span>{atlas.year} baseline</span></div></div>
